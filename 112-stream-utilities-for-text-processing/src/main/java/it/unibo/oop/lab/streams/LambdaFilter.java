...
public final class LambdaFilter extends JFrame {
    ...
    private enum Command {
        ...
        COUNT_CHARS("Count number of chars", str -> String.valueOf(str.length())),
        COUNT_LINES("Count number of lines", str -> Long.toString(str.chars().filter(e -> e == '\n').count() + 1)),
        LIST_IN_ALFA_ORDER("Words in alphabetical order", str -> 
            Arrays.stream(str.split(ANY_NON_WORD))
                .sorted()
                .collect(Collectors.joining("\n"))
        ),
        COUNT_EACH_WORD("Count for each word", str ->
            Arrays.stream(str.split(ANY_NON_WORD))
